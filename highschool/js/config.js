...
const CONFIG = {
  ...
  FIELD: {
    ...
    /* 1試合ごとに相手が強くなる幅の確率表（weight は出やすさ）。
       ...
       つまり地方大会の中では、勝ち進んでも難しさが変わらない */
    STEP: [
      { weight: 10, add: [-1.3,  0.0] },   // 楽な回（前より弱い）
      { weight: 22, add: [ 0.1,  1.0] },   // ほとんど変わらない
      { weight: 34, add: [ 1.0,  1.7] },   // ふつう
      { weight: 24, add: [ 1.7,  2.5] },   // 一段強い
      { weight: 10, add: [ 2.5,  3.8] },   // 妙に強い回
    ],
    ...
    NAT_STEP: [
      { weight:  8, add: [-2.5,  4.6] },   // 楽な回
      { weight: 22, add: [ 4.6,  7.7] },
      { weight: 34, add: [ 7.7, 10.3] },   // ふつう
      { weight: 26, add: [10.3, 12.8] },
      { weight: 10, add: [12.8, 16.5] },   // 妙に強い回
    ],
  ...
  GROWTH: {
    ...
    /* 投手の球速と変化球は、打撃の能力よりゆっくり伸ばす。
       毎試合のように上がると、3年で15km/hも速くなってしまう */
    VELO_CHANCE: 0.20,  // 投げた試合で球速が上がる確率（上がり幅はほぼ+1）
    PITCH_CHANCE: 0.10, // 投げた試合で変化球の切れ味が上がる確率。
                        // 新しい球種は覚えない（それは特訓の「新球習得」の役目）
  },
