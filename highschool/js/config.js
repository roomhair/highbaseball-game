...
const CONFIG = {
  ...
  /* 大会に出てくる相手の強さ（0〜100の目盛り）。
     ...
     STEP の平均は、勝ち上がっている間に自軍のチーム力が1試合あたり
     上がる幅（実測 +2.7）に合わせてある。つまり伸びたぶんだけ相手も
     上がるので、大会の中で差が勝手に開いたり縮んだりしない。
  ...
  FIELD: {
    ...
    national: { bonus: 0 },
    /* 1試合ごとに相手が強くなる幅の確率表（weight は出やすさ）。
       平均は +2.7。勝ち上がっている間に自軍のチーム力が
       1試合あたり上がる幅（実測 +2.7）と同じにしてある */
    STEP: [
      { weight: 10, add: [-2.5,  0.0] },   // 楽な回（前より弱い）
      { weight: 22, add: [ 0.2,  2.0] },   // ほとんど変わらない
      { weight: 34, add: [ 2.0,  3.4] },   // ふつう
      { weight: 24, add: [ 3.4,  5.0] },   // 一段強い
      { weight: 10, add: [ 5.0,  7.5] },   // 妙に強い回
    ],
