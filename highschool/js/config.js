...
const CONFIG = {
  ...
  FIELD: {
    ...
    /* 全国大会はこちらの表を使う。平均は +7.6 で、自軍の伸び（+2.7）より
       ずっと速い。地方大会の決勝から全国大会の1回戦へは期待値を
       動かさないので、全国大会の重さはこの表だけで作っている。
       勝ち進むほど苦しくなり、勝ち切れるのは10回に1回ほどになる */
    NAT_STEP: [
      { weight:  8, add: [-1.3,  4.1] },   // 楽な回
      { weight: 22, add: [ 4.1,  6.6] },
      { weight: 34, add: [ 6.6,  8.6] },   // ふつう
      { weight: 26, add: [ 8.6, 10.6] },
      { weight: 10, add: [10.6, 13.6] },   // 妙に強い回
    ],
  ...
  GROWTH: {
    ...
    HEAD_SPAN: 15,     // 100からの残りがこれ以上あるうちは満額
